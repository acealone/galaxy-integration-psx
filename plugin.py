...
class PlaystationPlugin(Plugin):
    def __init__(self, reader, writer, token):
        super().__init__(Platform.PlayStation, __version__, reader, writer, token)
    ...
    def do_auth(self):
        ...
        return Authentication("ePSXe", user_data["username"])
        ...
        for game in self.games:
            if str(game[1]) == game_id:
                if user_config.retroarch is not True:
                    subprocess.Popen([emu_path, "-loadbin", game[0], "-nogui", "-fullscreen"])
                    subprocess.Popen(
...
def main():
    create_and_run_plugin(PlaystationPlugin, sys.argv)
